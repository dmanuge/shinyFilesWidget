var shinyFiles = (function() {
      ...
      $('<div>').addClass('sF-modal modal-dialog').append(
        $('<div>').addClass('modal-content').append(
          $('<div>').addClass('modal-header').append(
            $('<button>', {text: 'Download', type: 'button', id: 'sF-selectButton'}).addClass('btn btn-primary')
            ).append(
 ...
 //   })
    ...
    modal.find('.sF-responseButtons #sF-cancelButton').on('click', function() {
      dismissFileChooser(button, modal); //removes modal, disables button
    })
    modal.find('.modal-header #sF-selectButton').on('click', function() {
        selectFiles(button, modal); //selects files
    })
    ...
    setTimeout(function() {
      if($('#shiny-modal').length ==0){
        modal.detach().appendTo('.content-data')
      }else{
